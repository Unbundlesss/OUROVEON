...
void DataBus::BusConfiguration::imgui( const DataBus& dataBus, const Fx::Provider::AbilityFlags flags )
{
    const auto& busNames = dataBus.m_busNames;

    if ( flags & Fx::Provider::kUsesValue )
        ImGui::DragFloat( "Value", &m_value, 0.05f, 0.0f, 1.0f, "%.2f" );
    
    if ( flags & Fx::Provider::kUsesTime )
        ImGui::DragFloat( "Time Scale", &m_timeScale, 0.1f, 0.0f, 10.0f, "%.2f" );

    if ( flags & Fx::Provider::kUsesBus1 )
        ImGui::Combo( "##bus_A", &m_indexA, busNames.data(), cBusCount );

    if ( flags & Fx::Provider::kUsesBus2 )
        ImGui::Combo( "##bus_B", &m_indexB, busNames.data(), cBusCount );

    if ( flags & Fx::Provider::kUsesRemapping )
    {
...
//            ImGui::PopItemWidth();
            ...
            Fx::Provider::AbilityFlags abilityFlags = Fx::Provider::kNothingSpecial;
            if ( m_busProviders[m_busEditIndex] != nullptr )
