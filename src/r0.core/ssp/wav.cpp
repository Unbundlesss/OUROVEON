    ...
    const uint32_t writeBufferInSeconds /* = 10 */ )
{
    ...
    fwrite( &nullHeader, sizeof( data::WavHeader ), 1, fp );
    ...
    auto instance = new WAVWriter( fp, bufferChunkSize * bufferChunksToAllocate, sampleRate );
    return std::unique_ptr<WAVWriter>( instance );
}
...
// ---------------------------------------------------------------------------------------------------------------------
WAVWriter::WAVWriter( FILE* fp, const size_t writeBufferChunkSize, const uint32_t sampleRate )
