...
std::string simplecpp::TokenList::stringify( bool emitHashLinePragmas ) const
{
        ...
        if (rawtok->op == '#' && !sameline(rawtok->previous, rawtok)) {
            ...
            if (ifstates.top() == TRUE && (rawtok->str() == ERROR || rawtok->str() == WARNING)) {
                if (outputList) {
                    ...
                    err.type = rawtok->str() == ERROR ? Output::SCPP_ERROR : Output::SCPP_WARNING;
                    err.location = rawtok->location;
