...
namespace simplecpp {
    ...
    struct SIMPLECPP_LIB Output {
        explicit Output(const std::vector<std::string> &files) : type(SCPP_ERROR), location(files) {}
        enum Type {
            SCPP_ERROR, /* #error */
            SCPP_WARNING, /* #warning */
            MISSING_HEADER,
    ...
    public:
        ...
        void dump() const;
        std::string stringify( bool emitHashLinePragmas ) const;
