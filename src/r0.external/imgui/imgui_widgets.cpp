...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (overlay_size.x > 0.0f)
        RenderTextClipped(ImVec2(ImClamp( 4.0f + /*fill_br.x +*/ style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, NULL, &overlay_size, ImVec2(0.0f, 0.5f), &bb);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool disableLabels = (flags & ImGuiSliderFlags_NoLabel) != 0;
    const ImVec2 noLabelOffset{ 0, disableLabels ? 6.0f : 0.0f };

    const ImVec2 label_size = disableLabels ? ImVec2(0, 10.0f) : CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + noLabelOffset, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
