...
namespace ImGui
{
    ...
    inline int Bezier( const char* label, float P[5] ) {
        // visuals
        static constexpr auto SMOOTHNESS = 64; // curve smoothness: the higher number of segments, the smoother curve
        static constexpr auto CURVE_WIDTH = 4; // main curved line width
        static constexpr auto LINE_WIDTH = 1; // handlers: small lines width
        static constexpr auto GRAB_RADIUS = 8; // handlers: circle radius
        static constexpr auto GRAB_BORDER = 2; // handlers: circle border width
        static constexpr auto AREA_CONSTRAINED = true; // should grabbers be constrained to grid area?
        static constexpr auto AREA_WIDTH = 128; // area width in pixels. 0 for adaptive size (will use max avail width)
        ...
        for ( int i = 0; i < 3; ++i ) {
            ...
            int idx = (int)(delta * (float)SMOOTHNESS);
